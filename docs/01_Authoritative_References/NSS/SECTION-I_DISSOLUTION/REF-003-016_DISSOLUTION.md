# REF-003-016 — Dissolution

**Document Family:** REF

**Document ID:** REF-003-016

**Title:** Dissolution

**Repository Path:**
`docs/01_Authoritative_References/NSS/SECTION-I_DISSOLUTION/`

**Parent Reference:** REF-003

**Source Reference:** REF-SRC-003

**Authority:** Primary Governing Authority

**Version:** 1.0

**Document Status:** APPROVED FOR FREEZE

**Business Rule Status:** FROZEN

---

# Revision History

| Version | Date | Description |
|----------|------------|-------------------------------------------|
| 1.0 | 2026-08-11 | Initial Governance By-Law Reference Register |

---

# Document Coverage

## Covered Sections

- Dissolution

---

# Source Information

**Primary Source**

Bye-Law of Nilachala Saraswata Sangha

**Edition**

Official Registered Bye-Law (Fourth Edition – 2025)

**Source Status**

Official Governing Source

**Repository Source ID**

REF-SRC-003

**Source File(s) in Repository**

`BY-LAW/NSS/NSS BYE-LAW.pdf` (cross-checked against `BY-LAW/NSS/NSS_Bye_Law.docx`)

---

# Introduction

This document serves as the authoritative reference register for the constitutional provisions relating to the Dissolution of the Kendra Sangha.

It preserves the original Governance Bye-Law provisions in a structured reference format without interpretation, modification or ERP implementation guidance.

Where ambiguity or inconsistency exists, the official Bye-Law shall prevail.

---

# Authority

REF-003-016 is an Authoritative Reference document.

It preserves the official Governance Bye-Laws without introducing ERP interpretation, implementation guidance or business rules.

Where ambiguity, inconsistency, or conflict exists, the official Bye-Law shall prevail.

---

# REF-003-016 — Dissolution

## Subject

Dissolution

---

### Source Reference

REF-SRC-003

---

### Constitutional Provision

**I. Dissolution:** Whenever it is observed that the Kendra Sangha with its Governing Body and Advisory Board is not functioning in its right earnest towards the fulfillment of the aims and objects of the Kendra Sangha, the same will be dissolved and upon dissolution, the properties that may remain after satisfying all debts and liabilities will vest in any registered Sakha Sangha of the Kendra Sangha which will undertake to fulfill the aims and objects laid down in article 'A' and the Memorandum.

The above provision is preserved in the source under the heading "I. Dissolution" as a single unlettered statement, not subdivided into further clauses in the official Bye-Law.

---

## Subject

Certification of the Bye-Law (following Dissolution)

---

### Source Reference

REF-SRC-003

---

### Constitutional Provision

Certified that this is a correct and true copy of the rules and regulations of the Nilachala Saraswata Sangha, Puri.

Sd./- Chaitanya Ch. Das : President

Sd./- Durga Charan Mohanty : Secretary and Parichalak

Sd./- Gunanidhi Patnaik : Vice-President

**Registration particulars:**

| Field | Value |
|-------|-------|
| Serial No. | 1 |
| No. in the Register | 7391/390 of 1973-74 |
| Name of the Society | Nilachala Saraswata Sangha |
| Nature of document | Memorandum |
| Date of Registration | 24-11-73 |

Sd./- P.C. Patnaik

For Registrar of Societies, Orissa.

The above provisions are preserved in the source as a certification/signature block appearing immediately after the Dissolution section, attesting to the accuracy of the Bye-Law as a whole. This block is distinct from (a) the certification block within the Memorandum of Association (REF-001-005), and (b) the Executive Body attestation appearing after the 1975 Resolutions (see the Resolution documents in this section) — all three are separate certification instances in the source and are preserved separately, not merged or deduplicated.

---

### Traceability

**Parent Reference**

REF-003

**Source Reference**

REF-SRC-003

**Derived Requirement IDs**

Pending

**Related References**

REF-001

REF-002

REF-003-001

REF-003-002

REF-003-010

**Related ERP Documents**

Pending

**Related Architecture**

Pending

**Related Design**

Pending

**Related Implementation**

Pending

**Related Test Cases**

Pending

---

### Verification

**Verification Status**

Verified

**Verified By**

Project Architecture Team

**Verification Source**

REF-SRC-003

**Last Verified**

2026-08-11

**Remarks**

Clause 1 refers to "article 'A' and the Memorandum" — corresponding to REF-001 (NSS Constitution), which preserves the Objects of the Society and the Memorandum of Association. This cross-reference is preserved verbatim from the official Bye-Law and shall not be reinterpreted.

---

# Related Governance

- REF-001 — NSS Constitution
- REF-002 — NSS Membership By-Laws
- REF-003-001 — Constitution of the Kendra Sangha
- REF-003-002 — Governing Body
- REF-003-010 — Advisory Board
- GOV-000 — Enterprise Governance Framework
- GOV-001 — Repository Governance & Documentation Architecture
- GOV-002 — Requirement Traceability Standard

---

# Document Authority

This document forms part of the **Authoritative Reference Library** of the NSS ERP Repository.

It preserves governing reference material derived from the official governing source.

Changes to this document require verification against the authoritative source document identified by **REF-SRC-003**.

Unverified modifications are prohibited.

Where any inconsistency exists between this document and the official governing source, the official governing source shall prevail.

---

**End of Document**
