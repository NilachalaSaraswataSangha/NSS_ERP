# REF-MS-10 — Settlement of Disputes

**Document Family:** REF-MS

**Document ID:** REF-MS-10

**Title:** Settlement of Disputes

**Repository Path:**
`docs/01_Authoritative_References/MAHILA_SANGHA/SECTION-K_DISPUTE_SETTLEMENT/`

**Parent Reference:** REF-MS-MOA

**Source Reference:** REF-SRC-MS

**Authority:** Primary Governing Authority (Nilachala Saraswata Mahila Sangha)

**Version:** 1.0

**Document Status:** APPROVED FOR FREEZE

**Business Rule Status:** FROZEN

---

# Revision History

| Version | Date | Description |
|----------|------------|-------------------------------------------|
| 1.0 | 2026-08-12 | Initial Authoritative Reference register, transcribed from `BY-LAW/NSS - Mahila Sangha/NSS_Mahila_Sangha_Bye_Law.docx`. |

---

# Document Coverage

## Covered Sections

- Settlement of disputes, if any (Bye-law, Clause 10)

---

# Source Information

**Primary Source**

Bye-Law of Nilachala Saraswata Mahila Sangha

**Source Status**

Official Registered Source

**Repository Source ID**

REF-SRC-MS

---

# Introduction

This document serves as the authoritative reference register for the Settlement of Disputes clause of the Bye-Law of Nilachala Saraswata Mahila Sangha. There is no directly equivalent standalone clause in the NSS Bye-Law itself (dispute resolution there is embedded within the Parichalak's disciplinary powers, see REF-003-009).

---

# Authority

REF-MS-10 is an Authoritative Reference document. It preserves the official Mahila Sangha Bye-Law without introducing ERP interpretation, implementation guidance, or business rules.

---

# REF-MS-10 — Settlement of Disputes

## Subject

Settlement of Disputes, if Any

---

### Source Reference

REF-SRC-MS

---

### Constitutional Provision

**10. Settlement of disputes, if any** — If any controversy arises in any matter pertaining to the affairs of Nilachala Kutir and the Sangha, the same should be referred to the Parichalak, Nilachala Saraswata Sangha. His decisions can not be challenged in any court of law and shall be held binding on all concerned.

The above provision is preserved in the source under the heading "10. Settlement of disputes, if any" as a single unlettered statement, not subdivided into further clauses.

---

### Traceability

**Parent Reference**

REF-MS-MOA

**Source Reference**

REF-SRC-MS

**Derived Requirement IDs**

Pending

**Related References**

REF-MS-6(iv) (Functions of the Parichalak)

REF-003-009 (NSS Duties of the Parichalak — comparable disciplinary authority)

---

### Verification

**Verification Status**

Verified

**Verified By**

Project Architecture Team

**Verification Source**

REF-SRC-MS

**Last Verified**

2026-08-12

**Remarks**

None

---

# Related Governance

- REF-MS-6(iv) — Functions of the Parichalak
- REF-003-009 — Duties of the Parichalak (NSS)
- AUTH-001 — Authoritative Reference Standard

---

# Document Authority

This document forms part of the **Authoritative Reference Library** of the NSS ERP Repository.

Changes to this document require verification against the authoritative source document identified by **REF-SRC-MS**.

Where any inconsistency exists between this document and the official governing source, the official governing source shall prevail.

---

**End of Document**
