...
Governance framework standards, all parented under GOV-001 (Status: Approved, v1.1.0).
...
- `GOV-001_Project_Governance_Principles.md` — top-level lifecycle hierarchy (Constitution →
  ...
  supremacy, roles, compliance rules, and the governance document Status lifecycle
  (`GOV-LIFE-006`: Draft → Review → Approved → Superseded/Retired). GDR is cross-cutting, not a
  sequential layer.
