...
column. The Local Sakha ERP ID (also called **ERP Number** — these
are two names for the same identity) belongs to the **affiliation
period**, not the permanent identity record. It resides on the
accepted `membership_sakha_affiliation` table (see §27.1).
...
```text
Format:  <3–5 character Organization/Sakha Short Code><sequence>
Example: ESS1192 (Ekamra Sakha), CTC42 (Cuttack Sakha)
```

The Local Sakha ERP ID is **auto-generated** by the system at
enrollment or transfer.

### Kendra Number — Not on This Table

The Kendra Number is the Kendra Sangha's registration reference,
carried on `parichaya_patra.document_number` (see §14). It is the
third identity tier and does not reside on `sangha_sevi`.
...
### Sangha Sevi ID vs. Local Sakha ERP ID vs. Kendra Number

These are three distinct identity tiers:

```text
Tier 1: Sangha Sevi ID          — permanent, NSS-wide, never changes, never reused
Tier 2: Local Sakha ERP ID      — Sakha-scoped (= ERP Number), one active per member,
                                   may be archived on transfer and reactivated on return
Tier 3: Kendra Number           — via parichaya_patra.document_number, Kendra-scoped
```
...
The Kendra Number resides on `parichaya_patra.document_number` (§14).
...
Stores the Membership-level Anumati Patra record. Applicable to
Probationary members only. Regular members retain historical EXPIRED
records. Associate members do not receive an Anumati Patra.
...
(Regular and Associate members). Probationary members do not receive a
Parichaya Patra.
