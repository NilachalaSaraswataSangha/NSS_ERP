...
The Local Sakha ERP ID is also referred to as the **ERP Number**. These
are two names for the same identity — the Sakha-scoped member number.
...
## 3.2 Kendra Number (Tier 3)

The Kendra Number is carried on the member's Parichaya Patra as the
`document_number`. It serves as the Kendra Sangha's own registration
...
**Physical representation:** `parichaya_patra.document_number`

## 3.3 Three-Tier Identity Summary

```text
Tier 1: Sangha Sevi ID          — NSS-wide, permanent
             sangha_sevi.sangha_sevi_id

Tier 2: Local Sakha ERP Number  — Sakha-scoped (= ERP Number)
             membership_sakha_affiliation.local_sakha_erp_id

Tier 3: Kendra Number           — Kendra-scoped, via Parichaya Patra
...
In the portal UI, `PROBATIONARY` is displayed as **"Darshaka"**
