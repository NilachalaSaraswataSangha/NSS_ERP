...
| `architecture/` | 13 cross-module architecture docs: `TECH_STACK_DECISIONS.md` (v1.3, approved tech decision record — FastAPI is now the sole backend framework, Django removed), `DEVELOPER_REFERENCE_GUIDE.md`, `BOOTSTRAP_ARCHITECTURE.md`, `CROSS_MODULE_PRINCIPLES.md`, `DDL_CREATION_ORDER.md`, `FK_DEPENDENCY_GRAPH.md`, `MODULE_DEPENDENCY_MAP.md`, `IMPLEMENTATION_DEPENDENCY_ORDER.md`, `FOUNDATION_API_CONTRACT.md` (v1.1, DRAFT — Tier 1 Foundation's 17-endpoint API contract), plus 4 Programmes & Events docs (`PROGRAMME_EVENT_DOMAIN_MODEL.md`, `EVENT_ENTITY_RECONCILIATION.md`, `PROGRAMMES_EVENTS_CROSS_MODULE_REVIEW.md`, `PROGRAMMES_EVENTS_RECONCILIATION_DECISIONS.md`); plus `code_explanations/` — 5 implementation walkthroughs/audits (Tier 0 vertical slice, Tier 1 Foundation, cross-tier security hardening, Tier 0 security audit, Tier 1 security audit) — see `architecture/README.md` for the per-file status table |
...
| `api/` | Reserved — no design content written yet. Note: FastAPI code now exists and runs at the root-level `api/` folder (Tier 0 bootstrap-RBAC endpoints + Tier 1 Foundation endpoints) — this `docs/03_Solution/api/` folder is a separate, still-empty documentation scaffold for that code's design |
