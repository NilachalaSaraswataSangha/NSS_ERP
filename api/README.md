...
behind a cross-tier security middleware stack (headers, opt-in CORS, rate limiting).
...
│                        that order), includes both routers, mounts `frontend/assets/` at
│                        `/assets`, serves `frontend/index.html` at `/` and
│                        `frontend/foundation.html` at `/foundation`, closes the DB pool on
│                        shutdown
