...
| `test_organization.py` | 72 | Tier 2 — all 7 `/api/v1/organization/*` endpoints across classes: `TestOrganizationTypes` (4), `TestStatuses` (4), `TestOrganizations` (27, incl. contact/online-presence field assertions and `limit`/`offset` pagination), `TestOrganizationChildren` (4), `TestChildrenStats`, `TestHierarchy` (9, incl. pagination and CTE depth-guard checks), `TestOrganizationUI` (13 page-structure checks for `/organization`), `TestOrganizationSecurity` (3) |
...
| `test_family.py` | 67 | Tier 4 — 7 `/api/v1/family/*` endpoints across 5 tables, incl. `TestOrgAdminFamilyFilter` and `TestSakhaAlignment`; no coverage yet for `/graph` or `/person/{pk}/membership-summary` |
| `test_membership.py` | 99 | Tier 4 — 7 `/api/v1/membership/*` endpoints (member list/detail/search, affiliations, Parichaya Patra, Anumati Patra, journey events); 1 known-failing test, `test_kumari_transition_has_event` (SS5 seed still needs a KUMARI_TRANSITION journey event) |
...
pytest tests/test_family.py       # Tier 4 (Family) only
pytest tests/test_membership.py   # Tier 4 (Membership) only
...
- `docs/03_Solution/api/API_CONTRACT.md` — Tier 4 (Family + Membership) API contract, 46
  endpoints total across all six tiers
